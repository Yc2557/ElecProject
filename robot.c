...
void setup_robot(struct Robot *robot){
    ...
    robot->crashed = 0;
    robot->auto_mode = 0;
    robot->rotationnumber = 0;
    robot->currentState = start;
    robot->turningRight = 0;
...
int checkRobotSensorLeftLeftAllWalls(struct Robot * robot, struct Wall_collection * head) {
    struct Wall_collection *ptr, *head_store;
    int i;
    double xDir, yDir;
    int robotCentreX, robotCentreY, xTL, yTL;
    ...
    {
        ptr = head_store;
        xDir = round(robotCentreX+(-ROBOT_WIDTH/2)*cos((robot->angle - 90)*PI/180)-(-ROBOT_HEIGHT/2-SENSOR_VISION-2+sensorSensitivityLength*i)*sin((robot->angle - 90)*PI/180));
        yDir = round(robotCentreY +(-ROBOT_WIDTH/2) *sin((robot->angle - 90)*PI/180)+(-ROBOT_HEIGHT/2-SENSOR_VISION-2+sensorSensitivityLength*i)*cos((robot->angle - 90)*PI/180));
        xTL = (int) xDir;
        yTL = (int) yDir;
        hit = 0;
...
void robotUpdate(struct SDL_Renderer * renderer, struct Robot * robot){
    ...
    */
    ...
    for (i = 0; i < 6; i++)
    {
        xDir = round(robotCentreX+(-ROBOT_WIDTH/2)*cos((robot->angle - 90)*PI/180)-(-ROBOT_HEIGHT/2-SENSOR_VISION-1+sensor_sensitivity*i)*sin((robot->angle - 90)*PI/180));
        yDir = round(robotCentreY +(-ROBOT_WIDTH/2) *sin((robot->angle - 90)*PI/180)+(-ROBOT_HEIGHT/2-SENSOR_VISION-1+sensor_sensitivity*i)*cos((robot->angle - 90)*PI/180));
        xTL = (int) xDir;
        yTL = (int) yDir;

        SDL_Rect rect = {xTL, yTL, 2, sensor_sensitivity+1};
        SDL_SetRenderDrawColor(renderer, 80+(20*(5-i)), 80+(20*(5-i)), 80+(20*(5-i)), 255);
        SDL_RenderDrawRect(renderer, &rect);
        SDL_RenderFillRect(renderer, &rect);
    }
...
void robotAutoMotorMove(struct Robot * robot, int front_left_sensor, int front_right_sensor, int left_left_sensor) {

    switch (robot->currentState) {
    case start:
        if (left_left_sensor == 0 || robot->turningRight == 1) {
            if (robot->rotationnumber < 6 && robot->turningRight == 0) {
                robot->direction = LEFT;
                robot->rotationnumber++;

            } else if ((front_left_sensor > 0) || (front_right_sensor > 0) || robot->turningRight == 1){
                robot->direction = DOWN;
                if (robot->turningRight == 0) {
                    robot->turningRight = 1;
                    robot->rotationnumber = 0;
                }
                if (robot->rotationnumber < 6) {
                    robot->direction = RIGHT;
                    robot->rotationnumber++;
                } else {
                    robot->currentState = stop;
                }
            } else if ((front_left_sensor == 0) && (front_right_sensor == 0) && (robot->rotationnumber >= 6)) {
                if (robot->currentSpeed<3) {
                    robot->direction = UP;
                }

            }
        } else if (robot->rotationnumber == 0){
            robot->currentState = stop;
        }
        break;
    case stop:
        ...
        if ((front_left_sensor == 0) && (front_right_sensor == 0) && left_left_sensor > 0) {
            robot->currentState = moveForward;
        } else if (robot->currentSpeed >= 0 && left_left_sensor == 0 && (front_left_sensor == 0 && front_right_sensor == 0)) {
            robot->currentState = turnLeft;
        } else if ((front_left_sensor >= 1 || front_right_sensor >= 1)) {
            robot->currentState = turnRight;
        }
        break;
    ...
    case moveForward:
        ...
        if ((front_left_sensor >= 1 || front_right_sensor >= 1)) {
            robot->currentState = stop;
        } else if (robot->currentSpeed>0 && left_left_sensor == 0 && (front_left_sensor == 0 && front_right_sensor == 0)) { //left turn
            robot->currentState = stop;
        }

        if (left_left_sensor > 0 && robot->rotationnumber >= 6)
            robot->rotationnumber = 0;
        if (left_left_sensor < 3 && left_left_sensor > 0 && (front_left_sensor == 0 && front_right_sensor == 0)) {
            printf("LJUST", robot->currentSpeed);
            robot->direction = LEFT;
            if (robot->currentSpeed < 4) {
                robot->direction = UP;
            }
    ...
    case turnLeft:
        printf("left");
        if (robot->rotationnumber < 6 && (front_left_sensor == 0 && front_right_sensor == 0)) {
            robot->direction = LEFT;
            robot->rotationnumber++;
        } else {
            if (robot->currentSpeed < 4) {
    ...
    /*
    if ((front_left_sensor == 0) && (front_right_sensor == 0) && left_left_sensor > 0 && (robot->rotationnumber == 0 || robot->rotationnumber >= 6)) {
        if (robot->currentSpeed<3)
            robot->direction = UP;

    }
    else if ((robot->currentSpeed>0) && (left_left_sensor == 0 || (front_left_sensor > 2 && front_right_sensor > 2)) && robot->rotationnumber == 0) {
        robot->direction = DOWN;
    }

    else if ((robot->currentSpeed>=0) && left_left_sensor == 0) {
        printf("%d\n", robot->rotationnumber);
        if (robot->rotationnumber < 6) {
            robot->direction = LEFT;
            robot->rotationnumber++;
    ...
    else if ((robot->currentSpeed>=0) && ((front_left_sensor >= 2) || (front_right_sensor >= 2)) && left_left_sensor > 0 ) {
        robot->direction = RIGHT;
    }

    if (robot->currentSpeed>0 && left_left_sensor > 0 && robot->rotationnumber >= 6) {
        robot->rotationnumber = 0;
    }
    */
