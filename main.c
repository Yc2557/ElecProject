...
int main(int argc, char *argv[]) {
    ...
    int front_left_sensor, front_right_sensor, left_left_sensor=0;
    ...
    while(!done){
        ...
        if (!robot.crashed){
            if (robot.auto_mode == 1)
                robotAutoMotorMove(&robot, front_left_sensor, front_right_sensor, left_left_sensor);
        ...
        //Otherwise compute sensor information
            ...
            left_left_sensor = checkRobotSensorLeftLeftAllWalls(&robot, head);
            if (left_left_sensor>0)
                printf("Left Sensor. Score = %d\n", left_left_sensor);
        }
